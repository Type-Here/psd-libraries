...
/*La ricerca del minimo dipende da n = T(n) = Theta(n) = n*/
int min_item(Item *a, int n) { 
	int min=0;
	int i;
	for(i=1;i<n;i++)
...
/*-- Dipende da n ricorsioni, nell'insieme: T(n) = O(n^2) = Omega(n^2) --*/
void selection_sort_rec(Item *a, int n) {
	int m;
	if(n <= 1) return;
	else{
		m=min_item(a, n); //Theta(n) = n;
		swap_item(&a[0], &a[m]); // O(1);
		selection_sort_rec(a+1,n-1); //Dipende da n ricorsioni: nell'insieme O(n^2)
	}
